...
class ActionDeltaNet(nnx.Module):
    def __init__(self, state_dim, context_dim, adv_dim, hidden, action_horizon, action_dim, rngs):
        in_dim = state_dim + hidden + adv_dim + 1
        self.context_proj = nnx.Linear(context_dim, hidden, rngs=rngs)
    ...
    def __call__(self, state, context, adv_vec, t):
        t_feat = t[:, None]
        context = jax.nn.silu(self.context_proj(context))
        x = jnp.concatenate([state, context, adv_vec, t_feat], axis=-1)
...
def load_v8_modules(recap_ckpt_dir):
    ...
    delta_net = ActionDeltaNet(
        state_dim=8, context_dim=CONTEXT_DIM, adv_dim=ADV_DIM, hidden=512,
...
def make_v8_infer_fn(model, adv_embed, delta_net, *, num_steps=10, beta=CFG_BETA):
    ...
    def build_kv_cache(m_state, obs):
        ...
        (prefix_out, _), kv_cache = m.PaliGemma.llm(
            [prefix_tokens, None], mask=prefix_attn_mask, positions=positions
        )
        prefix_weights = prefix_mask.astype(prefix_out.dtype)[..., None]
        context = jnp.sum(prefix_out * prefix_weights, axis=1) / jnp.maximum(
            jnp.sum(prefix_weights, axis=1), 1.0
        )
        return kv_cache, prefix_mask, context
    ...
    @jax.jit
    def run_delta(adv_st, delta_st, state_raw, context, adv_label_val, time_val):
        adv   = nnx.merge(adv_gdef,   adv_st)
        delta = nnx.merge(delta_gdef, delta_st)
        B = state_raw.shape[0]
        adv_vec = adv(jnp.full((B,), adv_label_val, dtype=jnp.int32))
        t_arr   = jnp.broadcast_to(time_val, B)
        return delta(state_raw, context, adv_vec, t_arr)  # [B, 50, 7]
    ...
    def infer(observation, rng):
        ...
        kv_cache, prefix_mask, context = build_kv_cache(model_state, observation)
        ...
        for _ in range(num_steps):
            ...
            d_cond   = run_delta(adv_state, delta_state, state_raw, context, jnp.int32(1), time_val)
            d_uncond = run_delta(adv_state, delta_state, state_raw, context, jnp.int32(0), time_val)
...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Comma-separated task IDs that receive the guided residual. "
            "All other tasks use the exact beta=0 path."
        ),
    )
    ...
    if args.active_task_ids:
        active_task_ids = {int(task_id) for task_id in args.active_task_ids.split(",")}
        if not active_task_ids.issubset(set(range(10))):
            raise ValueError(f"invalid task IDs: {sorted(active_task_ids)}")
    ...
    if args.eval_recap:
        ...
        beta_zero_infer_fn = None
        if active_task_ids is not None:
            beta_zero_infer_fn = make_v8_infer_fn(
                model, adv_embed, delta_net, num_steps=10, beta=0.0
            )
            print(
                f"Task gate: guided beta={args.cfg_beta} for "
                f"{sorted(active_task_ids)}; beta=0 elsewhere",
                flush=True,
            )
        ...
        for info in task_infos:
            ...
            guided = active_task_ids is None or task_id in active_task_ids
            infer_fn = recap_infer_fn if guided else beta_zero_infer_fn
            label = "RECAP-v9" if guided else "SFT-beta0"
            print(f"\n[{label}] Task {task_id}: {info['task_name']}", flush=True)
            succ, total = eval_task(
                task_id, info["task_name"], info["task_desc"],
                info["task_bddl"], info["init_states"],
                lambda obs, rng: infer_fn(obs, rng),
                tokenize, norm_stats, args.episodes_per_task, args.seed, label,
            )
            recap_results.append(succ / total)
            print(f"  Task {task_id} RECAP-v9: {succ}/{total} = {succ/total*100:.1f}%", flush=True)

        results["RECAP-v9"] = recap_results
        print(f"\nRECAP-v9 overall: {sum(recap_results)/len(recap_results)*100:.1f}%", flush=True)

    print("\n" + "=" * 65, flush=True)
    print(f"{'Task':<5} {'Name':<35} {'SFT':>6} {'RECAP-v9':>9}", flush=True)
    print("-" * 65, flush=True)
    for info in task_infos:
        tid  = info["task_id"]
        name = info["task_name"][:33]
        sr   = f"{results['SFT'][tid]*100:.0f}%"       if "SFT"      in results else "  -"
        rr   = f"{results['RECAP-v9'][tid]*100:.0f}%"  if "RECAP-v9" in results else "  -"
        print(f"{tid:<5} {name:<35} {sr:>6} {rr:>9}", flush=True)
    print("-" * 65, flush=True)
    sa = f"{sum(results['SFT'])/len(results['SFT'])*100:.1f}%"           if "SFT"      in results else "  -"
    ra = f"{sum(results['RECAP-v9'])/len(results['RECAP-v9'])*100:.1f}%" if "RECAP-v9" in results else "  -"
