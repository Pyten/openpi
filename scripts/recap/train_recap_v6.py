"""
Phase 1D v6: RECAP — suffix conditioning (adv_token in suffix, NOT prefix)

Key fix over v4/v5: adv_token is prepended to the SUFFIX (action denoising tokens),
not the prefix. This means the frozen backbone processes an identical prefix to SFT,
preserving positional encodings and attention patterns completely.

CFG at inference:
  - KV cache built from pure SFT prefix (no adv_token)
  - Each suffix step: adv_token prepended to suffix tokens
  - v_guided = v_uncond + beta*(v_cond - v_uncond)

Usage:
  cd /mnt/vepfs/pyten/Programs/code/pi0.6
  MUJOCO_GL=osmesa nohup .venv/bin/python -u scripts/recap/train_recap_v6.py \
    --sft-ckpt checkpoints/pi0_libero/recap_sft_baseline/29999 \
    --labeled-data data/rollouts/labeled_episodes_v5.pkl \
    --exp-name recap_policy_v6 \
    --num-steps 20000 --batch-size 64 --adv-lr 1e-3 \
    --ortho-lambda 0.1 --norm-lambda 0.001 --fsdp-devices 4 \
    > logs/recap_train_v6.log 2>&1 &
"""
...
HIDDEN_DIM       = 1024  # suffix token dim (not LLM dim)
...
def compute_loss_adv_only(
    ...
    """Flow matching loss with adv_token in SUFFIX (not prefix)."""
    ...
    adv_tokens = adv_embed(effective_labels)[:, None, :]  # (B, 1, HIDDEN)

    # prepend adv_token to suffix
    suffix_tokens_aug  = jnp.concatenate([adv_tokens, suffix_tokens], axis=1)
    suffix_mask_aug    = jnp.concatenate([jnp.ones((B, 1), dtype=bool), suffix_mask], axis=1)
    suffix_ar_mask_aug = jnp.concatenate([jnp.array([True]), suffix_ar_mask])

    # attention mask: full sequence = prefix + suffix_aug
    input_mask = jnp.concatenate([prefix_mask, suffix_mask_aug], axis=1)
    ar_mask    = jnp.concatenate([prefix_ar_mask, suffix_ar_mask_aug], axis=0)
    attn_mask  = make_attn_mask(input_mask, ar_mask)
    positions  = jnp.cumsum(input_mask, axis=1) - 1

    (_, suffix_out), _ = model.PaliGemma.llm(
        [prefix_tokens, suffix_tokens_aug],
        mask=attn_mask,
        positions=positions,
        adarms_cond=[None, adarms_cond],
    )
    # adv_token is first suffix token; action tokens follow
...
def main():
    ...
    parser.add_argument("--exp-name",      default="recap_policy_v6")
    ...
    log(f"Config: steps={args.num_steps}, batch={args.batch_size}, "
        ...
        f"norm_lambda={args.norm_lambda}  [FROZEN BACKBONE, SUFFIX CONDITIONING]")
    ...
    log(f"Starting RECAP v6 training from step {start_step+1} to {total_steps}...")
    ...
    for step in range(start_step + 1, total_steps + 1):
        ...
        if step % SAVE_INTERVAL == 0 or step == total_steps:
            ...
            if model_params_dir.exists():
                ...
                _sh.rmtree(model_params_dir)
            if adv_embed_dir.exists():
                import shutil as _sh
                _sh.rmtree(adv_embed_dir)
    ...
    log("RECAP v6 training complete.")
