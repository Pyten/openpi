...
CONTEXT_DIM    = 2048   # frozen PaliGemma prefix hidden width
SAVE_INTERVAL  = 1000
LOG_INTERVAL   = 50
...
class ActionDeltaNet(nnx.Module):
    """Residual MLP conditioned on frozen visual-language prefix features."""

    def __init__(self, state_dim: int, context_dim: int, adv_dim: int, hidden: int,
                 action_horizon: int, action_dim: int, rngs: nnx.Rngs):
...
def compute_loss(model, adv_embed, delta_net, rng,
    ...
    delta_7  = delta_net(states_raw, context, adv_vecs, time)  # [B, H, 7]
