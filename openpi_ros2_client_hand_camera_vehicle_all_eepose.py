...
DEFAULT_EEPOSE_URDF_PATH = (
    Path(__file__).resolve().parent
    / "our_data"
    / "eepose"
    / "tianyi2_urdf-tianyi2.0"
    / "urdf"
    / "tianyi2.0_URDF.urdf"
)


class EeposeKinematicsAdapter:
    """Convert between robot joint states and 14-dim TCP poses in the head frame."""
    ...
    LEFT_ARM_STATE_SLICE = slice(2, 9)
    RIGHT_ARM_STATE_SLICE = slice(15, 22)
    HEAD_STATE_IDXS = [0, 1]
    LEFT_TIP_FRAME = "left_tcp_link"
    RIGHT_TIP_FRAME = "right_tcp_link"
    REFERENCE_FRAME = "head_roll_link"

    def __init__(self, urdf_path: str | Path = DEFAULT_EEPOSE_URDF_PATH) -> None:
        try:
            import pinocchio as pin  # type: ignore
        except ModuleNotFoundError as exc:
            raise RuntimeError("eepose 客户端需要 Pinocchio，请先安装 `pin`/`pinocchio`。") from exc
        ...
        self.neutral_q = np.asarray(pin.neutral(self.model), dtype=np.float64)

        self.reference_frame_id = self._frame_id(self.REFERENCE_FRAME)
        self.left_tip_frame_id = self._frame_id(self.LEFT_TIP_FRAME)
        self.right_tip_frame_id = self._frame_id(self.RIGHT_TIP_FRAME)
        self.left_q_indices = self._joint_q_indices(self.LEFT_ARM_JOINTS)
        self.right_q_indices = self._joint_q_indices(self.RIGHT_ARM_JOINTS)
        self.left_v_indices = self._joint_v_indices(self.LEFT_ARM_JOINTS)
        self.right_v_indices = self._joint_v_indices(self.RIGHT_ARM_JOINTS)
        self.head_q_indices = self._joint_q_indices(self.HEAD_JOINTS)

    def _frame_id(self, frame_name: str) -> int:
        frame_id = self.model.getFrameId(frame_name)
        if frame_id >= len(self.model.frames):
            raise KeyError(f"Frame {frame_name!r} not found in URDF")
        return int(frame_id)
    ...
    def _joint_q_indices(self, joint_names: List[str]) -> List[int]:
        ...
        for joint_name in joint_names:
            ...
            if joint_id >= len(self.model.joints):
                raise KeyError(f"Joint {joint_name!r} not found in URDF")
            indices.append(int(self.model.idx_qs[joint_id]))
        return indices
    ...
    def _q_from_state32(self, state32: np.ndarray) -> np.ndarray:
        state32 = np.asarray(state32, dtype=np.float64)
        if state32.shape != (32,):
            raise ValueError(f"state32 must be shape (32,), got {state32.shape}")

        q = self.neutral_q.copy()
        for q_idx, value in zip(self.head_q_indices, state32[self.HEAD_STATE_IDXS], strict=True):
            q[q_idx] = float(value)
        for q_idx, value in zip(self.left_q_indices, state32[self.LEFT_ARM_STATE_SLICE], strict=True):
            q[q_idx] = float(value)
        for q_idx, value in zip(self.right_q_indices, state32[self.RIGHT_ARM_STATE_SLICE], strict=True):
            q[q_idx] = float(value)
        return q

    def joint_state32_to_eepose14(self, state32: np.ndarray) -> np.ndarray:
        q = self._q_from_state32(state32)
        self.pin.forwardKinematics(self.model, self.data, q)
        self.pin.updateFramePlacements(self.model, self.data)

        reference_pose = self.data.oMf[self.reference_frame_id]
        left_pose = reference_pose.actInv(self.data.oMf[self.left_tip_frame_id])
        right_pose = reference_pose.actInv(self.data.oMf[self.right_tip_frame_id])
        return np.concatenate([self._se3_to_xyz_quat(left_pose), self._se3_to_xyz_quat(right_pose)]).astype(np.float32)

    def eepose14_to_arm_joints(
        self,
        eepose14: np.ndarray,
        seed_state32: np.ndarray,
        *,
        max_iters: int = 80,
        tol: float = 1e-4,
        damping: float = 1e-4,
        ...
        eepose14 = np.asarray(eepose14, dtype=np.float64).reshape(-1)
        if eepose14.shape != (14,):
            raise ValueError(f"eepose14 must be shape (14,), got {eepose14.shape}")

        q = self._q_from_state32(seed_state32)
        self.pin.forwardKinematics(self.model, self.data, q)
        self.pin.updateFramePlacements(self.model, self.data)
        reference_pose = self.data.oMf[self.reference_frame_id]
        left_target = reference_pose.act(self._pose_from_xyz_quat(eepose14[:7]))
        right_target = reference_pose.act(self._pose_from_xyz_quat(eepose14[7:]))

        ok_left = ok_right = False
        for _ in range(max_iters):
            q, ok_left = self._ik_step(q, self.left_tip_frame_id, left_target, self.left_v_indices, tol, damping, step_size)
            q, ok_right = self._ik_step(q, self.right_tip_frame_id, right_target, self.right_v_indices, tol, damping, step_size)
            if ok_left and ok_right:
                break

        left = np.asarray([q[idx] for idx in self.left_q_indices], dtype=np.float32)
        right = np.asarray([q[idx] for idx in self.right_q_indices], dtype=np.float32)
        return np.concatenate([left, right]), bool(ok_left and ok_right)
    ...
    def _ik_step(
        ...
        err = self.pin.log(current.inverse() * target).vector
        if float(np.linalg.norm(err)) < tol:
            return q, True

        jacobian = self.pin.computeFrameJacobian(self.model, self.data, q, frame_id, self.pin.ReferenceFrame.LOCAL)
        jacobian = jacobian[:, v_indices]
        jj_t = jacobian @ jacobian.T
        delta = jacobian.T @ np.linalg.solve(jj_t + (damping**2) * np.eye(6), err)
        velocity = np.zeros((self.model.nv,), dtype=np.float64)
        for v_idx, dv in zip(v_indices, delta, strict=True):
            velocity[v_idx] = -step_size * float(dv)

        q = self.pin.integrate(self.model, q, velocity)
        if self.model.lowerPositionLimit.size == self.model.nq:
            q = np.clip(q, self.model.lowerPositionLimit, self.model.upperPositionLimit)
        return q, False

    def _pose_from_xyz_quat(self, xyz_quat: np.ndarray):
        quat = np.asarray(xyz_quat[3:7], dtype=np.float64)
        norm = float(np.linalg.norm(quat))
        if norm <= 1e-8:
            raise ValueError(f"Invalid near-zero quaternion in eepose action: {quat}")
        quat = quat / norm
        return self.pin.SE3(self.pin.Quaternion(quat).matrix(), np.asarray(xyz_quat[:3], dtype=np.float64))

    def _se3_to_xyz_quat(self, pose) -> np.ndarray:
        quat = self.pin.Quaternion(pose.rotation)
        quat.normalize()
        return np.asarray([*pose.translation, *np.asarray(quat.coeffs()).reshape(-1)], dtype=np.float32)
...
class OpenPIRos2Client(Node):
    ...
    - 状态维度: 14 (head_roll_link 坐标系下左右 TCP 位姿)
    - 动作维度: 14 (head_roll_link 坐标系下左右 TCP 位姿)
    - 相机: 
    ...
    def __init__(
        ...
        # 模型接口配置：state 为双臂14维关节角，action 为14维 eepose。
        self.state_dim = 14
        self.action_dim = 14
        self.image_height = 224
        self.image_width = 224
        self.eepose_kinematics = EeposeKinematicsAdapter()
        self.latest_state32_for_ik: Optional[np.ndarray] = None
        logger.info(
            "已启用 eepose 模式: state=双臂14维关节角, action=14维eepose, 参考坐标系=%s, URDF=%s",
            EeposeKinematicsAdapter.REFERENCE_FRAME,
            self.eepose_kinematics.urdf_path,
        )
    ...
    def build_observation(self) -> Optional[Dict[str, Any]]:
        ...
        # 构建原始32维关节状态；模型输入只使用双臂14维关节角。
        state32 = self.build_state_vector()
        self.latest_state32_for_ik = state32.copy()
        state = np.concatenate([state32[2:9], state32[15:22]]).astype(np.float32)
        ...
        logger.debug(
            "Built observation keys: %s; arm14_state.shape=%s, cam_high.shape=%s, cam_left_wrist.shape=%s, cam_right_wrist.shape=%s",
    ...
    def infer_actions(self, observation: Dict[str, Any]) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if self.print_inference_actions:
                logger.info(
                    "推理生成动作 actions shape=%s (行=horizon步, 列=14维 eepose):\n%s",
                    actions.shape,
    ...
    def _flush_actions_session(self) -> None:
        ...
        try:
            ...
            with open(txt_path, "w") as fout:
                fout.write(f"# 会话 eepose state + action 序列（{len(self._session_npy_list)} 次推理）\n")
                ...
                for i, (state, action) in enumerate(zip(self._session_state_list, self._session_npy_list)):
                    ...
                    fout.write("# STATE (14-dim eepose)\n")
    ...
    def _save_action_information_json(
        ...
        try:
            ...
            payload = {
                "description": (
                    "OpenPI policy eepose actions (full horizon rows x 14 dims). "
                    "Values are converted to arm joints by IK before publishing."
                ),
                "prompt": self.prompt,
    ...
    def _trigger_inference(self):
        ...
        for action in actions:
            ...
            # 发布 eepose 动作：先 IK 成双臂关节，再只发布 /arm/cmd_pos。
            self._publish_eepose_action_vector(last_action)
        # 发布动作向量
    ...
    def _publish_initial_action(self):
        ...
        logger.info("📤 发布初始双臂关节值（仅 /arm/cmd_pos），prompt='%s'，间隔 %.2f s", self.prompt, interval)
        # 启动初值
        ...
        if prompt_norm == "pick up the box":
            # 抓取初值
            self._publish_arm_joints_from_state32(self.initial_action_2)
            time.sleep(interval)
            self._publish_arm_joints_from_state32(self.initial_action_3)
            time.sleep(interval)
        else:
            logger.info("prompt 不是 'pick up the box'，跳过初始值发布。")

    def _publish_eepose_action_vector(self, eepose_action: np.ndarray) -> None:
        """Convert one 14-dim eepose action to arm joints and publish only /arm/cmd_pos."""
        if self.latest_state32_for_ik is None:
            logger.warning("缺少 IK 初值 state32，跳过 eepose action 发布")
            return
        try:
            arm_joints, ik_ok = self.eepose_kinematics.eepose14_to_arm_joints(
                eepose_action,
                self.latest_state32_for_ik,
            )
        except Exception as e:
            logger.warning("eepose14 转双臂关节失败，跳过该 action: %s", e)
            return

        if not ik_ok:
            logger.warning("eepose IK 未收敛，跳过该 action，eepose=%s", np.array2string(eepose_action, precision=5))
            return

        next_seed = self.latest_state32_for_ik.copy()
        next_seed[2:9] = arm_joints[:7]
        next_seed[15:22] = arm_joints[7:]
        self.latest_state32_for_ik = next_seed
        self._publish_arm_joint_vector(arm_joints)

    def _publish_arm_joints_from_state32(self, state32: np.ndarray) -> None:
        """Publish only arm joints from a 32-dim preset/current state."""
        state32 = np.asarray(state32, dtype=np.float32).reshape(-1)
        if state32.shape != (32,):
            logger.warning("预设 state/action 不是 32 维，无法抽取双臂关节: shape=%s", state32.shape)
            return
        self._publish_arm_joint_vector(np.concatenate([state32[2:9], state32[15:22]]))

    def _publish_arm_joint_vector(self, arm_joints14: np.ndarray) -> None:
        """Publish left/right 7-DoF arm joint targets to /arm/cmd_pos."""
        arm_joints14 = np.asarray(arm_joints14, dtype=np.float32).reshape(-1)
        if arm_joints14.shape != (14,):
            logger.warning("双臂关节命令不是 14 维，跳过发布: shape=%s", arm_joints14.shape)
            return

        if self.arm_cmd_pub is None:
            logger.debug("CmdSetMotorPosition 不可用，跳过 /arm/cmd_pos 发布")
            return
        if CmdSetMotorPosition is None or SetMotorPosition is None:
            logger.debug("bodyctrl_msgs 不可用，跳过 /arm/cmd_pos 发布")
            return
        ...
        try:
            ...
            for i, name in enumerate(range(11, 18)):
                ...
                cmd_item.cur = float(cur_limit_list[i])
                arm_msg.cmds.append(cmd_item)

            for i, name in enumerate(range(21, 28)):
                cmd_item = SetMotorPosition()
                cmd_item.name = int(name)
                raw_pos = float(arm_joints14[7 + i])
                if name in self.joint_limits:
                    min_rad, max_rad = self.joint_limits[name]
                    cmd_item.pos = float(np.clip(raw_pos, min_rad, max_rad))
                else:
            ...
            self._apply_arm_post_limits(arm_msg.cmds)
            self.arm_cmd_pub.publish(arm_msg)
            logger.debug(
                "发布 /arm/cmd_pos: 左臂=%s, 右臂=%s",
                [f"{x:.3f}" for x in arm_joints14[:7]],
                [f"{x:.3f}" for x in arm_joints14[7:]],
            )
        except Exception as e:
            logger.warning(f"发布 /arm/cmd_pos 失败: {e}")

    def _apply_arm_post_limits(self, cmds: List[Any]) -> None:
        """Keep the original arm-specific safety rule after basic joint clipping."""
        joint_positions = {cmd.name: cmd.pos for cmd in cmds}
        threshold = 0.2
        ...
        if (
            left_joint_11 is not None
            and left_joint_12 is not None
            and all(pos is not None for pos in left_joints_13_17)
            and abs(left_joint_11) <= threshold
            and all(abs(pos) <= threshold for pos in left_joints_13_17)
            and abs(left_joint_12) <= threshold
        ):
            new_pos_12 = threshold + 0.01 if left_joint_12 >= 0 else -(threshold + 0.01)
            for cmd in cmds:
                if cmd.name == 12:
        ...
        if (
            right_joint_21 is not None
            and right_joint_22 is not None
            and all(pos is not None for pos in right_joints_23_27)
            and abs(right_joint_21) <= threshold
            and all(abs(pos) <= threshold for pos in right_joints_23_27)
            and abs(right_joint_22) <= threshold
        ):
            new_pos_22 = threshold + 0.01 if right_joint_22 >= 0 else -(threshold + 0.01)
            for cmd in cmds:
                if cmd.name == 22:
